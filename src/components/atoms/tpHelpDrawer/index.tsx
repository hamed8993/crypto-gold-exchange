...
interface TpHelpDrawerProps {
  isOpen: boolean;
  onClose: () => void;
}

function TpHelpDrawer({ isOpen, onClose }: TpHelpDrawerProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-between px-8 py-2 pb-4">
        ...
        >
          {t("tpHelpDescription1")}
        </p>
        <p
          dir="rtl"
          className="w-[90%] text-justify text-xs/6 text-accentText dark:text-accentTextDark"
        >
          {t("tpHelpDescription2")}
        </p>
        <Image
          alt="Abstract charts image"
          className="w-full max-w-[1200px] scale-90 animate-revealClip md:top-12 lg:top-20 xl:top-16"
          height={66}
          src={"/assets/images/tp.webp"}
          width={135}
        />
        <p
          dir="rtl"
          className="w-[90%] mb-4  mt-3 text-justify text-xs/6 text-mainText dark:text-mainTextDark"
        >
          {t("tpHelpDescription3")}
        </p>
