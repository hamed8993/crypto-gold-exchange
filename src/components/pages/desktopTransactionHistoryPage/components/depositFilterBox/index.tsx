...
import CurrencySelect from "@/components/molecules/paymentCurrencySelect";
...
import { useDepositHistoryFilterContext } from "../depositHistoryTab/provider";

function DepositFilterBox() {
  const { locale } = useUrl();
  const t = useTranslations();

  const [dateValue, setDateValue] = useState<string>("");

  const { setValue, control, watch } = useDepositHistoryFilterContext();
    ...
    >
      <div className="mt-2 mb-2 grid w-full grid-cols-[repeat(auto-fit,minmax(100px,250px))] items-end gap-3 px-2">
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <CurrencySelect onChange={onChange} value={value} />
          )}
        />
...
export default DepositFilterBox;
