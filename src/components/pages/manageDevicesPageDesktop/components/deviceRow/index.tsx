...
import SureLogoutDeviceModal from "../manageDevicesTab/components/sureLogoutDeviceModal";
...
function DeviceRow({ currentSession, data, refetch }: DeviceRowProps) {
  const [isSureOneLogoutModalOpen, setIsSureOneLogoutModalOpen] =
  ...
  const { mutate, isPending } = useDeleteUser_dataDelete_active_sessions({
    onSuccess: (data) => {
      ...
      refetch();
      setIsSureOneLogoutModalOpen(false);
    },
    onError: (error) => {
  ...
  return (
    <>
      <div className="flex min-h-10 w-full items-center justify-between p-2">
        <div
          className="flex h-10 w-10 cursor-pointer items-center justify-center"
          onClick={() => {
            if (data?.uuid === currentSession) return;

            setItemUuid(data?.uuid || "");
            setIsSureOneLogoutModalOpen(true);
          }}
        >
      ...
      {isSureOneLogoutModalOpen && (
        <SureLogoutDeviceModal
          isPendingApi={isPending}
          isModalOpen={isSureOneLogoutModalOpen}
          setIsModalOpen={setIsSureOneLogoutModalOpen}
          text={t("sureLogoutThisDevice")}
          operation={() => onRemove(itemUuid)}
        />
      )}
    </>
  );
