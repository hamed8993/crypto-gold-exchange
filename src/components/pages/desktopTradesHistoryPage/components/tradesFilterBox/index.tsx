...
import { useTradeHistoryFilterContext } from "../../provider";

function TradeFilterBox() {
  const { locale } = useUrl();
  const t = useTranslations();

  const [dateValue, setDateValue] = useState<string>("");

  const { setValue, control, watch } = useTradeHistoryFilterContext();
      ...
      <div className="mt-2 mb-2 grid w-full grid-cols-[repeat(auto-fit,minmax(100px,250px))] items-end gap-3 px-2">
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <CustomInput
              inputClassName="focus-visible:border-mainText  placeholder:text-xs placeholder:text-accentText "
              maxLength={100}
